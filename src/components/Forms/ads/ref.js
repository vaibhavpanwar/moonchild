...
const WorkerCreateAd = (props) => {
  ...
  const renderCountry = (data) => {
    return (
      <TouchableOpacity
        ...
        style={[
          LocalStyles.countryContainer,
          {
            width: (width - s(64)) / 3,
            borderColor:
              country === data.item._id ? Colors.primary : Colors.white,
          },
        ]}
        onPress={() =>
          setCountry(country === data.item._id ? '' : data.item._id)
        }>
        <View style={LocalStyles.countryImageView}>
          <FastImage
            resizeMode={FastImage.resizeMode.contain}
            style={[
              LocalStyles.countryImage,
        ...
        <View style={LocalStyles.countryTextView}>
          <Text
            numberOfLines={2}
            style={[
              LocalStyles.countryText,
              {
  ...
  const renderCountryEmpty = () => {
    return (
      <View style={Styles.container}>
        <View style={LocalStyles.emptyTextContainer}>
          {countriesList.length ? null : (
              ...
              <TouchableOpacity
                ...
                <Image
                  ...
                  source={
                    ...
                    answers.find((e) => e.questionId === data.item._id)
                      ?.optionId
                      ? require('@images/ic_boolean_selected.png')
                      : require('@images/ic_boolean_unselected.png')
                  }
                />
              </TouchableOpacity>
              <View style={Styles.widthSeperator16} />
  ...
  const booleanImageSelector = (questionId, selectedOption) => {
    if (
      selectedOption ===
      answers.find((e) => e.questionId === questionId)?.optionId
    ) {
  ...
  const fetchCountries = () => {
    if (!netStatus) {
      ...
      axios
        .get(`${Config.apiUrl}${Config.apiUserTag}listCountry`)
        .then((response) => {
          console.log(response);
          if (response.status == 200) {
            if (response.data.data.listing.length) {
              props.actions.setCountries(response.data.data.listing);
            }
          } else {
  ...
  const nextStep = (itemNumber) => {
    if (itemNumber === 1) {
      ...
      let validation = Validation.validate(ValidationRulesStep2());
      if (validation.length != 0) {
        return props.actions.showOptionsAlert(validation[0]);
      } else {
        getQuestions();
  ...
  const getQuestions = () => {
    if (!netStatus) {
      ...
      axios
        .get(
          `${Config.apiUrl}${Config.apiUserTag}listQuestions?userType=2&categoryId=${category}&subCategoryId=${subCategory}`,
        )
        .then((response) => {
          console.log(response);
          if (response.status == 200) {
              ...
              <View style={LocalStyles.stepsView}>
                ...
                <Text
                  style={[
                    LocalStyles.stepsText,
                    {
                      fontFamily: step > 3 ? fontFamilyMedium : fontFamilyBold,
            ...
            {step === 1 ? (
              <View style={Styles.container}>
                <Text
                  ...
                  {string('WhereWantGetWork')}
                </Text>
                <FlatList
                  key={'country'}
                  style={[Styles.container, {marginHorizontal: s(8)}]}
                  keyboardShouldPersistTaps="handled"
...
export default connect(mapStateToProps, mapDispatchToProps)(WorkerCreateAd);
